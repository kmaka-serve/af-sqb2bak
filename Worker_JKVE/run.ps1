...
function Upload-FromFile([string]$url, [string]$filePath) {
  ...
  try {
    ...
    if (-not ($filePath.ToLower().EndsWith('.log'))) {
      # якщо це не лог — ставимо octet-stream і x-ms-blob-type
    ...
    try { $r.EnsureSuccessStatusCode() | Out-Null } finally { $r.Dispose() }
    return $code
  } finally { $fs.Dispose(); $c.Dispose(); $h.Dispose() }
}
...
$SRC_SHARE_DIR        = "C:\home\SqbFiles\JKVE"
$SRC_BLOB             = "___FULL_us_jkve_multi_replica_2025081517.SQB"

$DST_CONTAINER        = "backups"
$DST_BLOB             = "JKVE/FULL_us_jkve_multi_replica_2025081517.bak"  # шлях через '/'

$SQL_PASSWORD         = "gNVuyCyUOI*3@3xqoX)evJjbO5Ok4*7u"
$TOOLS_SAS_URL        = ""
